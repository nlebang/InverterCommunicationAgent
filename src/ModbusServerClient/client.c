...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1) {
        if (strcmp(argv[1], "1") == 0) {
            // clientTamper = 1;
            updateClientTamper(1);
        } else if (strcmp(argv[1], "2") == 0) {
            updateServerTamper(1);
            // serverTamper = 1;
    ...
    /* Single */
    printf("\nTest for coil writing\n");
    if(clientTamper) {
        printf("Client signature tampering enabled\n");
    }
    rc = modbus_write_bit(ctx, UT_BITS_ADDRESS, ON);
    if(clientTamper && rc == -1) {
        printf("Tamper detected by the server\n");
    }
...
// goto close;
    ...
    printf("\nTest for coil reading\n");
    if(serverTamper) {
        printf("Server signature tampering enabled\n");
    }
    rc = modbus_read_bits(ctx, UT_BITS_ADDRESS, 1, tab_rp_bits);
    if(serverTamper && rc == -1) {
        printf("Tamper detected by the client\n");
    }
